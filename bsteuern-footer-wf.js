/* ============================================================
   ...
   Version 2.2 · für den im Webflow Designer gebauten Footer
...
(function () {
  ...
  var CONFIG = {
    ...
    /* Klasse des unsichtbaren Stil-Ankers. Alles darin wird bei der
       Elementsuche uebersprungen — sonst greift das Skript auf einen
       leeren Anker-Div zu, falls dieser durch Duplizieren des echten
       Bandes entstanden ist und dessen data-bs-Attribut geerbt hat. */
    anchorClass: 'stil-anker'
  };
  ...
  /* ----------------------------------------------------------
     ...
     ---------------------------------------------------------- */
  /* Sucht ein Element mit data-bs="name" und ueberspringt dabei alles,
     was im Stil-Anker liegt. Meldet Duplikate in der Konsole. */
  function pick(root, name) {
    var sel = '[data-bs="' + name + '"]';
    var nodes = root.querySelectorAll(sel);
    var treffer = null, imAnker = 0;

    for (var i = 0; i < nodes.length; i++) {
      var el = nodes[i];
      var drin = el.closest ? el.closest('.' + CONFIG.anchorClass) : null;
      if (drin) { imAnker++; continue; }
      if (!treffer) treffer = el;
    }

    if (imAnker && window.console && console.warn) {
      console.warn('[bsteuern-footer] ' + imAnker + ' Element(e) mit data-bs="' + name +
        '" liegen im Stil-Anker und wurden uebersprungen. ' +
        'Entferne dort das Attribut data-bs — es stammt vermutlich vom Duplizieren des echten Elements.');
    }
    return treffer;
  }
