...
(function () {
  ...
  function istDunkel(hex) {
    if (!hex) return true; // Fallback: Indigo gilt als dunkel
    hex = hex.replace('#', '');
  ...
  /* Themen einer Karte ermitteln — zwei Quellen, in dieser Reihenfolge:

     1. Verschachtelte Collection List (Multi-Reference-Feld).
        Webflow kann Multi-Reference nicht in Attribute binden, deshalb rendert
        eine Collection List die Themen als Textelemente mit Klasse .thema-name.
     2. Fallback: Attribut data-topic mit · oder Komma getrennt (Plain-Text-Feld).

     Ergebnis wird pro Element gecacht, damit Filtern nicht jedes Mal das DOM liest. */
  function themenVon(el) {
    if (el.__themen) return el.__themen;

    var liste = [];
    var knoten = el.querySelectorAll('.thema-name');
    if (knoten.length) {
      Array.prototype.forEach.call(knoten, function (k) {
        var t = (k.textContent || '').trim();
        if (t && liste.indexOf(t) === -1) liste.push(t);
      });
    } else {
      var roh = el.getAttribute('data-topic');
      if (roh) {
        roh.split(/[·,]/).forEach(function (t) {
          t = t.trim();
          if (t && liste.indexOf(t) === -1) liste.push(t);
        });
      }
  ...
  function initRaster() {
    ...
    /* Kontrastfarbe pro Person aus --k-akzent ableiten.
       Die Akzentfarbe selbst kommt aus dem CMS (Custom
       Attribute style="--k-akzent:#…"), fest pro Person.
       Hier wird nur --k-kontrast gesetzt: dunkle Akzente
       bekommen cremefarbenen Text, helle inkfarbenen. */
    items.forEach(function (el, i) {
      el.setAttribute('data-tid', 't' + i);
      // Webflow blockiert das style-Attribut, daher kommt die Akzentfarbe
      // über data-akzent (CMS-Feld Akzent-Hex). Hier auf --k-akzent übertragen.
      ...
      el.style.setProperty('--k-kontrast', dunkel ? '#F5EFE3' : '#0E0C1C');
      /* Badge-Farbe auf der Vorderseite: Akzent der Person,
         bei hellen Akzenten (Lemon) stattdessen Ink. */
      el.style.setProperty('--k-badge', dunkel ? (akzent || '#3D2BD5') : '#0E0C1C');
      /* Randfarbe der Badges auf der Rueckseite: heller Rand auf
         dunklem Akzent, dunkler Rand auf hellem Akzent. */
      el.style.setProperty('--k-rand',
        dunkel ? 'rgba(245,239,227,0.35)' : 'rgba(14,12,28,0.25)');

      /* "+N weitere" unter den zwei sichtbaren Badges */
      var mehr = el.querySelector('.thema-mehr');
      var leer = el.querySelector('.thema-leer');
      var anzahl = themenVon(el).length;
      if (mehr) mehr.textContent = anzahl > 2 ? '+' + (anzahl - 2) + ' weitere' : '';
      if (leer) leer.style.display = anzahl === 0 ? '' : 'none';
    });
    ...
    items.forEach(function (el) {
      themenVon(el).forEach(function (t) {
    ...
    function render() {
      var sichtbar = reihenfolge.filter(function (el) {
        return aktivThema === ALLE || themenVon(el).indexOf(aktivThema) !== -1;
      });
