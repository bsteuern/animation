...
(function () {
  ...
  /* Steigt durch Wrapper hindurch, bis ein Knoten mit mehreren
     Kindern erreicht ist. Ohne das liefert <a><div><h3>Titel</h3>
     <span>Vollzeit</span></div></a> ein einziges, verklebtes
     Textstück. */
  ...
  /* Sammelt die Textstücke eines Knotens EINZELN ein, statt
     textContent zu nehmen. Gemessen an der echten Homerun-Ausgabe:
     zwei benachbarte <span> ohne Leerzeichen dazwischen ergeben
     sonst „Steuerberater/inVollzeit" beziehungsweise „RemoteVollzeit".
     ausser = Knoten, der übersprungen wird (der Link selbst). */
  ...
  function leseJobs(html) {
    ...
    Array.prototype.forEach.call(doc.querySelectorAll('a[href]'), function (a) {
      /* Der Link enthält oft mehr als den Titel — bei b'steuern etwa
         zusätzlich die Anstellungsart. Erstes Stück ist der Titel,
         alles Weitere wandert in die Unterzeile. */
      ...
      /* Standort steht als Geschwister neben dem Link */
  ...
  function baueZeile(vorlage, job) {
    ...
    /* Reihenfolge: ist der Wurzelknoten selbst ein <a>, ist er der Link.
       querySelector findet den Wurzelknoten NICHT — ohne diesen ersten
       Zweig bliebe die Zeile ohne href und damit unklickbar, sobald der
       Vorlage die Klasse .rolle-zeile fehlt. */
    var link = (knoten.tagName === 'A')
      ? knoten
  ...
  /* Prüft die Vorlage einmal beim Start und benennt Mängel im Klartext.
     Ohne das äußern sich fehlende Klassen als „sieht kaputt aus" ohne
     jede Fehlermeldung — gemessen der teuerste Fehlerfall. */
  function pruefeVorlage(vorlage) {
    if (!window.console || !console.warn) return;
    var mangel = [];

    if (vorlage.tagName !== 'A') {
      mangel.push('ist kein Link Block, sondern <' +
        String(vorlage.tagName).toLowerCase() + '> \u2014 nicht klickbar');
    }
    if (!vorlage.classList.contains('rolle-zeile')) {
      mangel.push('Klasse .rolle-zeile fehlt \u2014 Layout, Trennlinie und Hover greifen nicht');
    }
    if (!vorlage.querySelector('.rolle-titel')) mangel.push('.rolle-titel fehlt');
    if (!vorlage.querySelector('.rolle-sub'))   mangel.push('.rolle-sub fehlt');
    if (!vorlage.querySelector('.pfeil-svg'))   mangel.push('.pfeil-svg fehlt');

    if (mangel.length) {
      console.warn('[team-page] Vorlage unvollst\u00E4ndig \u2014 ' + mangel.join(' \u00B7 ') +
        '. Siehe Kapitel 7b.4.');
    }
  }
