...
(function () {
  ...
  function bindDropdown(ddEl, options, onSelect) {
    ...
    const list    = ddEl.querySelector('[role="listbox"]');
    const $value  = ddEl.querySelector('.dropdown-trigger [class*="dropdown-list"]:not([role])') || ddEl.querySelector('.dropdown-trigger [class*="dropdown-list"]');
  ...
  function positionBubble(trigger, bubble) {
    ...
    const arrowLeft = Math.max(8, rect.left + rect.width / 2 - left - 6);
  ...
  function bindTooltips() {
    document.querySelectorAll('.tooltip-trigger').forEach(t => {
      t.addEventListener('click', e => {
        ...
        if (!isOpen) {
          ...
          const bubble = t.nextElementSibling;
          if (bubble && bubble.classList.contains('tooltip-bubble')) {
            requestAnimationFrame(() => positionBubble(t, bubble));
          }
        }
      });
    });
    ...
    window.addEventListener('resize', () => {
      document.querySelectorAll('.tooltip-trigger[aria-expanded="true"]').forEach(t => {
        const bubble = t.nextElementSibling;
        if (bubble && bubble.classList.contains('tooltip-bubble')) positionBubble(t, bubble);
      });
    });
  }
