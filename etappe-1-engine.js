/* ════════════════════════════════════════════════════════════
   b'STEUERN PREISRECHNER · ENGINE · v31
   Page Settings → Before </body> → in <script>…</script>

   ÄNDERUNGEN v31:
   - updateCompare() neu integriert (fehlte in v30)
   - STB_MULTIPLIERS pro Rechtsform (1.4 / 1.6 / 1.9)
   - updateCompare in updateSummary() eingehängt
   - Holdings: Compare-Sektion korrekt versteckt
   - Raten-Sektion: style.display als Fallback für
     Webflow-CSS-Override (display:none via Klasse)
   ════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function updateSummary() {
    ...
    if (ratenContent) {
      ...
      ratenContent.style.display = (vormonate === 0) ? 'none' : '';
    }
    if (ratenEmpty) {
      ratenEmpty.hidden        = (vormonate > 0);
      ratenEmpty.style.display = (vormonate > 0) ? 'none' : '';
    }
