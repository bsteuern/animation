...
(function () {
  ...
  function bindFloatingBar() {
    ...
    /* Webflow display:none überschreiben — transform steuert die Sichtbarkeit */
  ...
  function bindBarDetailsLink() {
    ...
    /* Tap auf Preis-Bereich: expandieren / kollabieren */
    link.addEventListener('click', e => {
      ...
      if (isExpanded) {
        ...
        bar._collapseTimer = setTimeout(() => bar.classList.remove('is-expanded'), 5000);
      }
    });

    /* Tap außerhalb → kollabieren */
    document.addEventListener('click', () => {
      bar.classList.remove('is-expanded');
      clearTimeout(bar._collapseTimer);
    });
