...
(function () {
  ...
  /* ─── TOOLTIPS ─── */
  /* Bubble ist position:fixed — wir positionieren via getBoundingClientRect().
     Das umgeht alle overflow:hidden / z-index / Viewport-Breiten-Probleme. */
  ...
  function showTooltip(trigger) {
    ...
    /* Horizontale Position: bündig mit rechter Kante des Triggers,
       aber innerhalb des Viewports halten */
    let bRight = window.innerWidth - rect.right - gap;
    if (bRight < gap) bRight = gap;

    /* Vertikale Position: Bubble öffnet ÜBER dem Trigger */
    const bBottom = window.innerHeight - rect.top + gap;

    bubble.style.bottom       = bBottom + 'px';
    bubble.style.right        = bRight  + 'px';
  ...
  function bindTooltips() {
    ...
    /* Schließen wenn gescrollt (verhindert Drift bei fixed-Position) */
