/* ════════════════════════════════════════════════════════════
   b'STEUERN PREISRECHNER · ENGINE · v34
   Page Settings → Before </body> → in <script>…</script>

   ÄNDERUNGEN v34:
   - Default-Paket: 'sorglos' statt 'selbststarter'
     (Mittelpaket ist vorausgewählt wie im Prototype)
   ════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function parsePackagesFromCMS() {
    ...
    nodes.forEach(node => {
      ...
      if (!form) return;
      const badge = node.dataset.packageBadge || '';
      const pkg = {
        ...
        comingSoon: badge.toLowerCase().includes('verfügbar'),
  ...
  const state = {
    ...
    package:      'sorglos',   /* Mittelpaket vorausgewählt wie im Prototype */
  ...
  function renderPackageCards() {
    ...
    /* Sicherstellen dass das ausgewählte Paket in dieser Form existiert */
    const exists = group.packages.find(p => p.id === state.package && !p.comingSoon);
    if (!exists) {
      const fallback = group.packages.find(p => !p.comingSoon);
      if (fallback) state.package = fallback.id;
    }
  ...
  function bindForm() {
    ...
    tiles.forEach(tile => {
      tile.addEventListener('click', () => {
        ...
        /* Bei Formwechsel: Mittelpaket bevorzugen, sonst erstes verfügbares */
        if (gr) {
          const sorglos = gr.packages.find(p => p.id === 'sorglos' && !p.comingSoon);
          const fallback = gr.packages.find(p => !p.comingSoon);
          state.package = (sorglos || fallback)?.id || state.package;
        }
    ...
    $('step1PillClose')?.addEventListener('click', () => {
      state.form = 'einzelunternehmer'; state.package = 'sorglos';
