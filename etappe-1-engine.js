...
(function () {
  ...
  function renderCompareTable() {
    ...
    const tier = pkg => pkg.id === 'selbststarter' ? 'grundpaket'
    ...
    /* Label mit optionalem Tooltip */
    const renderLabel = f => {
      if (!f.tooltip) return f.label;
      const tipId = `tipFeat-${f.key}`;
      return `${f.label}<span class="tooltip-wrap tooltip-wrap--inline">
        <div class="tooltip-trigger" id="${tipId}" role="button" tabindex="0" aria-label="Mehr Info zu ${f.label}" aria-expanded="false">i</div>
