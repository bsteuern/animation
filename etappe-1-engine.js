...
(function () {
  ...
  function renderBarSheet({ basePrice, txCost, surcharge, vormonate,
    ...
    /* Globale Hilfsfunktion für Raten-Pills (state ist im IIFE-Scope) */
    window._bsSetRaten = function(n) { state.ratenCount = n; updateSummary(); };

    const fmt = v => formatEuro(v);
    const row = (label, value, muted) =>
      `<div class="bar-sheet-row">
        <span${muted ? ' style="color:rgba(14,12,28,.5)"' : ''}>${label}</span>
        <span class="bar-sheet-val">${value}</span>
      </div>`;
    ...
    /* Details-Link — Pfad zur vollen Summary-Card */
    h += `<div class="bar-sheet-details-link-wrap">
      <span class="bar-sheet-details-link">Details zur Kalkulation ↑</span>
    </div>`;

    /* Details-Link zur vollen Summary-Card */
    const detailsLink = el.querySelector('.bar-sheet-details-link');
    detailsLink?.addEventListener('click', () => {
      /* Sheet schließen */
      const bar = $('floatingBar');
      bar?.classList.remove('is-expanded');
      if (bar?._overlay) { bar._overlay.style.opacity = '0'; bar._overlay.style.pointerEvents = 'none'; }
      /* Summary aufklappen und dorthin scrollen */
