...
(function () {
  ...
  function renderBarSheet({ basePrice, txCost, surcharge, vormonate,
    ...
    if (vormonate > 0) {
      ...
      h += `<div class="bar-sheet-row"><span style="color:rgba(14,12,28,.55)">Monatliche Rate</span>
            <span style="font-weight:600">${fmt(vormonateRate)}</span></div>`;
      h += `<div style="font-size:.68rem;color:rgba(14,12,28,.45);margin-top:2px">
            Befristet auf ${state.ratenCount} Monatsraten</div>`;
    }
  ...
  function bindFloatingBar() {
    ...
    /* ── Bottom Sheet per JS erstellen — keine Webflow-Änderungen nötig ── */
    const inner = bar.querySelector('.floating-bar-inner') || bar;

    /* Sheet-Container vor dem Preis-Bereich einfügen */
    const sheet = document.createElement('div');
    sheet.id = 'barSheet';
    sheet.innerHTML = '<div class="bar-sheet-handle"></div><div id="barSheetContent"></div>';
    inner.prepend(sheet);
    ...
    /* Overlay hinter Sheet, vor Seiteninhalt */
    const overlay = document.createElement('div');
    overlay.id = 'barOverlay';
    overlay.style.cssText = [
      'position:fixed','inset:0','background:rgba(14,12,28,.35)',
  ...
  function bindBarDetailsLink() {
    ...
    const open = () => {
      ...
      clearTimeout(bar._collapseTimer);
      bar._collapseTimer = setTimeout(close, 8000);
    };
    const close = () => {
