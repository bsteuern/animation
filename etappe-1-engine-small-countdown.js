...
(function () {
  ...
  function renderActionBanner() {
    ...
    /* Banner als ERSTES Kind der Card (#calculatorSummary) — NICHT in .summary-body,
       ...
       Es bricht per CSS-Margin aus dem Card-Padding aus (full-bleed). */
  ...
  function calculate() {
    ...
    return { basePrice, txCost, surcharge, revTier, totalMonthly };
  }
  ...
  function updateSummary() {
    ...
    if (!calc) return;
    const { basePrice, txCost, surcharge, revTier, totalMonthly } = calc;
  ...
  function renderBarSheet() {
    ...
    const calc = calculate();
    if (!calc) return;
    const { basePrice, txCost, surcharge } = calc;
    const pkg     = getCurrentPkg();
