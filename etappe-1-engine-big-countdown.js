/* ════════════════════════════════════════════════════════════
   b'STEUERN PREISRECHNER · ENGINE · v37
   Page Settings → Before </body> → in <script>…</script>

   ÄNDERUNGEN v38:
   - Confetti vollständig entfernt (fireConfetti/bindConfetti + .confetti-canvas CSS)
   - Globale Leaks beseitigt: keine Funktionen mehr ausserhalb der IIFE
   - Typeform-Prefill gerettet & in die IIFE verlegt -> bindCtaPrefill()
     (CTA schreibt weiterhin alle 9 Kalkulator-Felder in die Typeform-URL)
   - kein preventDefault-Delay mehr: CTA navigiert direkt auf die Prefill-URL

   ÄNDERUNGEN v37 (TARIFAKTION, temporär bis Monatsende):
   - aktionEnde()/aktionAktiv(): Aktion läuft bis Monatsende 23:59:59
   - renderActionBanner(): gelbes Countdown-Banner oben in der Summary-Card
     + kompakter Streifen in der Floating-Bar (Mobile)
   - Odometer-Flip (WAAPI) je Ziffer, respektiert prefers-reduced-motion
   - Auto-Abschaltung: nach Monatsende verschwindet alles automatisch
   - pkgPrice() + NEUE_PREISE{}: Preis-Override ab Aktionsende (Werte folgen)
     greift in calculate(), renderPackageCards() und renderCompareTable()
...
(function () {
  ...
  const COMPARE_FEATURES = [
    { key: 'jahresabschluss', label: 'Jahresabschluss (EÜR/Bilanz)',                     selbststarter: true,      sorglos: true,      vollservice: true      },
    { key: 'steuern',        label: 'Alle betrieblichen Steuererklärungen',               selbststarter: true,      sorglos: true,      vollservice: true      },
    { key: 'feststellung',   label: 'Gesonderte Feststellung',     onlyForms: ['personengesellschaft'], selbststarter: true, sorglos: true, vollservice: true },
    { key: 'guthaben',       label: 'Beratungsguthaben p.a.',                             selbststarter: '100 €',   sorglos: '200 €',   vollservice: '300 €'   },
  ...
  /* ════════════════════════════════════════════════════════════
     TARIFAKTION (temporär · v37)
     Läuft bis Monatsende. Danach blendet der Countdown Banner +
     Floating-Bar-Streifen automatisch aus und rechnet mit NEUE_PREISE
     weiter — kein Republish nötig.
     ════════════════════════════════════════════════════════════ */

  /* Aktionsende = letzter Tag des aktuellen Monats, 23:59:59 (lokale Zeit). */
  function aktionEnde() {
    const d = new Date();
    return new Date(d.getFullYear(), d.getMonth() + 1, 0, 23, 59, 59, 999).getTime();
  }
  function aktionAktiv() { return Date.now() <= aktionEnde(); }

  /* ▼▼▼ NOCH ZU BEFÜLLEN — neue (höhere) Preise ab dem 1. des Folgemonats.
     Key = package.id, Wert = neuer Monats-GRUNDPREIS in €.
     Solange leer/auskommentiert: kein Anstieg (Fallback auf CMS-Preis).
     Gilt aktuell NUR für den monatlichen Grundpreis je Paket — Onboarding,
     Transaktions-Tiers und Startup-Tarife bleiben unverändert. */
  const NEUE_PREISE = {
    // selbststarter: 0,
    // sorglos:       0,
    // vollservice:   0,
  ...
  /* Aktuell gültiger Grundpreis eines Pakets:
     während Aktion → CMS-Preis (= Aktionspreis),
     nach Aktion    → NEUE_PREISE[id] falls gesetzt, sonst CMS-Preis. */
  function pkgPrice(pkg) {
    if (!pkg) return 0;
    if (!aktionAktiv() && NEUE_PREISE[pkg.id] != null) return NEUE_PREISE[pkg.id];
    return pkg.price;
  }

  /* ─── Countdown-Rendering ─── */
  function aktionTimerHTML() {
    const unit = (i, lbl) =>
      `<span class="action-banner__unit">
         <span class="action-banner__mask"><span class="action-banner__digits" data-cd="${i}">--</span></span>
         <span class="action-banner__label">${lbl}</span>
       </span>`;
    const sep = `<span class="action-banner__sep">:</span>`;
    return unit(0, 'Tage') + sep + unit(1, 'Std') + sep + unit(2, 'Min');
  }

  function aktionRemaining() {
    let diff = Math.max(0, Math.floor((aktionEnde() - Date.now()) / 1000));
  ...
  function renderActionBanner() {
    if (!aktionAktiv()) return;

    /* Banner als ERSTES Kind der Card (#calculatorSummary) — NICHT in .summary-body,
       sonst wird es Flex-Geschwister von summary-meta/reset und verdrängt sie.
       Es bricht per gemessenem Card-Padding aus (full-bleed). */
    const card = document.getElementById('calculatorSummary')
              || document.querySelector('.calculator-summary');
    if (card && !card.querySelector('.action-banner')) {
      const banner = document.createElement('div');
      banner.className = 'action-banner';
      banner.setAttribute('data-action-banner', '');
      banner.innerHTML =
        `<span class="action-banner__text"><span class="action-banner__text-em">Aktuellen Tarif sichern</span> – bevor die Preise steigen.</span>
         <span class="action-banner__timer" role="timer" aria-label="Aktion endet in">${aktionTimerHTML()}</span>`;
      card.prepend(banner);
      /* Full-bleed ohne Raten: echtes Card-Padding messen und exakt ausbrechen.
         Webflow blockt style-Attribute im Designer — JS-style auf JS-Elementen ist ok. */
      try {
        const cs = getComputedStyle(card);
        banner.style.marginTop   = '-' + cs.paddingTop;
        banner.style.marginLeft  = '-' + cs.paddingLeft;
        banner.style.marginRight = '-' + cs.paddingRight;
      } catch (e) {}
    }

    /* Kompakter Streifen am oberen Rand der Floating-Bar (nur Mobile sichtbar) */
    const barInner = document.querySelector('#floatingBar .floating-bar-inner');
    if (barInner && !barInner.querySelector('.action-bar-strip')) {
      const strip = document.createElement('div');
      strip.className = 'action-bar-strip';
      strip.setAttribute('data-action-strip', '');
      strip.innerHTML =
        `<span>Aktuellen Tarif sichern · endet in</span>` +
        `<span class="action-bar-strip__time" data-bar-time>–</span>`;
      barInner.prepend(strip);
    }
  }

  function aktionFlip(el) {
    if (!el || PREFERS_REDUCED || !el.animate) return;
    el.animate(
  ...
  function aktionTick(prev) {
    if (!aktionAktiv()) { aktionEnd(); return null; }
    const cur = aktionRemaining();

    document.querySelectorAll('.action-banner__timer').forEach(timer => {
      for (let i = 0; i < 3; i++) {
        const el = timer.querySelector(`[data-cd="${i}"]`);
        if (!el) continue;
        if (!prev || prev[i] !== cur[i]) { el.textContent = cur[i]; aktionFlip(el); }
      }
    });

    const barTime = document.querySelector('[data-bar-time]');
    if (barTime) barTime.textContent = `${parseInt(cur[0], 10)}T ${cur[1]}:${cur[2]}`;

    return cur;
  }

  let aktionTimerId = null;
  function aktionEnd() {
    document.querySelectorAll('[data-action-banner], [data-action-strip]').forEach(el => el.remove());
    if (aktionTimerId) { clearInterval(aktionTimerId); aktionTimerId = null; }
    /* Preise neu rechnen — greift NEUE_PREISE, sobald befüllt */
    renderPackageCards();
    updateSummary();
  }

  function startActionCountdown() {
    if (!aktionAktiv()) { aktionEnd(); return; }
    renderActionBanner();
    let prev = aktionTick(null);
    aktionTimerId = setInterval(() => { prev = aktionTick(prev); }, 1000);
  }
  ...
  function calculate() {
    ...
    const basePrice     = pkgPrice(pkg);
    const txCost        = txSurcharge(state.transactions);
    const revTier       = getRevenueTier(state.revenue);
    const surcharge     = (basePrice + txCost) * (revTier.percent / 100);
    const totalMonthly  = basePrice + txCost + surcharge;
  ...
  function renderCell(val) {
    if (val === true)  return '<span class="compare-table__check" aria-label="enthalten"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="square" stroke-linejoin="miter" style="display:block;width:100%;height:100%;"><path d="M5 12l5 5L20 6"/></svg></span>';
      ...
      <div class="compare-table__head">
        ...
        ${pkgs.map(p => `
          <div class="compare-table__head-cell ${p.comingSoon ? 'is-coming-soon' : ''}">
            ...
            <span class="compare-table__pkg-price">${p.comingSoon ? 'Demnächst' : `ab ${priceCeil(pkgPrice(p))} €<small>/ Mo</small>`}</span>
          </div>`).join('')}
      </div>`;

    const visibleFeatures = COMPARE_FEATURES.filter(f => !f.onlyForms || f.onlyForms.includes(state.form));
        ...
        ${pkgs.map(p => `
          <details class="compare-card ${p.comingSoon ? 'is-coming-soon' : ''}" ${p.badge === 'Empfehlung' ? 'open' : ''}>
            <summary class="compare-card__head">
              ...
              <span class="compare-card__pkg-price">${p.comingSoon ? 'Demnächst' : `ab ${priceCeil(pkgPrice(p))} €<small> / Mo</small>`}</span>
  ...
  function renderPackageCards() {
    ...
    wrap.innerHTML = group.packages.map(pkg => {
      ...
      let hint = '';
      /* Gründer-Tarif aktiv: kein Prozent-Badge — wird in Summary-Card angezeigt */
      ...
      return `<div class="package-card${pkg.comingSoon ? ' is-coming-soon' : ''}"
        ...
        <div class="package-card-price">
          ${pkg.comingSoon
            ? `<span class="package-card-price-value package-card-price-soon">Demnächst</span>`
            : `<span class="package-card-price-prefix">ab</span>
               <span class="package-card-price-value">${fmtV(pkgPrice(pkg))}</span>
  ...
  function updateSummary() {
    ...
    /* ── FLOATING BAR ── */
    animateValue($('barMonthly'), state.displayed.barMonthly, totalMonthly, 400, formatEuro);
    state.displayed.barMonthly = totalMonthly;

    /* Asterisk: sichtbar wenn Vormonate vorhanden */
    toggleEl($('barAsterisk'), vormonate > 0);

    /* Sheet-Inhalt aktualisieren wenn sichtbar */
    if ($('floatingBar')?.classList.contains('is-expanded')) renderBarSheet();
  ...
  function renderHoldings() {
    ...
    animateValue($('barMonthly'), state.displayed.barMonthly, HOLDINGS_PRICE, 400, formatEuro);
    state.displayed.barMonthly = HOLDINGS_PRICE;
    const barPkgLineEl  = $('barPkgLine');
    const barPkgLineWrap = $('barPkgLineWrap');
    if (barPkgLineEl) barPkgLineEl.textContent = 'Holdings · individuelle Lösung';
    if (barPkgLineWrap) { barPkgLineWrap.removeAttribute('hidden'); barPkgLineWrap.style.display = ''; }
  }
  ...
  /* ─── BAR SHEET RENDER ─── */
  /* Globale Hilfsfunktion für Raten-Pills — einmal registrieren */
  window._bsSetRaten = function(n) { state.ratenCount = n; renderBarSheet(); };

  /* Self-contained: berechnet alles selbst, wird auch in open() aufgerufen */
  function renderBarSheet() {
    const el = $('barSheetContent');
    if (!el) return;

    if (state.form === 'holdings') {
      el.innerHTML = '<div class="bar-sheet-section" style="text-align:center;padding:1.25rem;"><p style="font-size:.85rem;color:rgba(14,12,28,.6)">Holdings · individuelle Kalkulation im Erstgespräch.</p></div>';
      return;
    }

    const calc = calculate();
    if (!calc) return;
    const { basePrice, txCost, surcharge, vormonate, vormonateTotal, vormonateRate } = calc;
    const pkg     = getCurrentPkg();
    const pkgName = pkg ? pkg.name : '';

    const fmt = v => formatEuro(v);
    const row = (label, val, muted) =>
      `<div class="bar-sheet-row"><span${muted ? ' style="color:rgba(14,12,28,.5)"' : ''}>${label}</span><span class="bar-sheet-val">${val}</span></div>`;

    let h = '';

    h += `<div class="bar-sheet-section">`;
    h += `<p class="bar-sheet-label">Monatlich</p>`;
    h += row(pkgName, fmt(basePrice));
    if (txCost > 0)    h += row('Transaktionen', '+' + fmt(txCost), true);
    if (surcharge > 0) h += row('Umsatz-Aufschlag', '+' + fmt(surcharge), true);
    h += `</div>`;

    h += `<div class="bar-sheet-section">`;
    h += `<p class="bar-sheet-label">Einmalig</p>`;
    h += row('Onboarding', fmt(ONBOARDING));
    h += `</div>`;

    if (vormonate > 0) {
      h += `<div class="bar-sheet-section">`;
      h += `<p class="bar-sheet-label">Ratenzahlung</p>`;
      h += row('<strong>' + vormonate + '</strong> Vormonate', fmt(vormonateTotal));
      h += `<p class="bar-sheet-sub">Aufteilung</p>`;
      h += '<div class="bar-sheet-pills">' + [1,3,6].map(r =>
        '<div class="bar-sheet-pill' + (r === state.ratenCount ? ' is-active' : '') +
        '" onclick="window._bsSetRaten(' + r + ')">' + r + ' Mo</div>'
      ).join('') + '</div>';
      h += row('Monatliche Rate', fmt(vormonateRate));
      h += `<p class="bar-sheet-sub">Befristet auf ${state.ratenCount} Monatsraten</p>`;
      h += `</div>`;
    }

    h += `<div class="bar-sheet-details-link-wrap"><span class="bar-sheet-details-link">Details zur Kalkulation ↑</span></div>`;

    /* HTML ins DOM schreiben */
    el.innerHTML = h;

    el.querySelector('.bar-sheet-details-link')?.addEventListener('click', () => {
      const bar = $('floatingBar');
      bar?.classList.remove('is-expanded');
      if (bar?._overlay) { bar._overlay.style.opacity = '0'; bar._overlay.style.pointerEvents = 'none'; }
      document.body.style.overflow = ''; /* Scroll-Lock aufheben */
      const summaryEl = $('calculatorSummary');
      if (summaryEl) {
        summaryEl.dataset.mobileCollapsed = 'false';
        $('summaryToggle')?.setAttribute('aria-expanded', 'true');
        const lbl = $('summaryToggleLabel');
        if (lbl) lbl.textContent = 'Details schließen';
        setTimeout(() => {
          window.scrollTo({ top: summaryEl.getBoundingClientRect().top + window.scrollY - 80, behavior: 'smooth' });
        }, 150);
      }
    });
  }
  ...
  function bindFloatingBar() {
    ...
    /* Sheet nur erstellen wenn noch nicht im DOM */
    let sheet = document.getElementById('barSheet');
    if (!sheet) {
      ...
      if (!document.getElementById('barSheetContent')) {
        const ce = document.createElement('div');
        ce.id = 'barSheetContent';
        sheet.appendChild(ce);
      }
      const handle = sheet.querySelector('.bar-sheet-handle');
      if (handle && !handle.id) handle.id = 'barSheetHandle';
    }

    /* CTA in Sheet verschieben */
    const cta = inner.querySelector('.floating-bar-cta');
    if (cta && !sheet.contains(cta)) sheet.appendChild(cta);
    ...
    /* Handle schließt Sheet + Body Scroll entsperren */
    ...
    /* IntersectionObserver: Bar verstecken wenn Summary sichtbar
       + Sheet schließen damit Overlay nicht hängen bleibt */
    if (sum) {
  ...
  function bindBarDetailsLink() {
    ...
    /* Tap außerhalb → schließen
       composedPath() statt contains() — path wird vor DOM-Änderungen aufgezeichnet
       Fallback auf bar.contains() für ältere Browser */
    document.addEventListener('click', e => {
      const inBar = e.composedPath ? e.composedPath().includes(bar) : bar.contains(e.target);
      if (!inBar) close();
    });
  }
  ...
  /* ─── TYPEFORM PRE-FILL ─── */
  /* Baut die Typeform-URL mit allen Kalkulator-Daten als Hidden-Field-Parameter.
     Bleibt als window.bsteuernTypeformURL exposed (evtl. von anderen Embeds genutzt);
     intern konsumiert von bindCtaPrefill(). */
  window.bsteuernTypeformURL = function(baseURL) {
    if (!baseURL || baseURL === '#') return baseURL;
    const calc   = state.form !== 'holdings' ? calculate() : null;
    const params = new URLSearchParams({
      rechtsform:    FORM_LABELS[state.form] || state.form,
      paket:         state.package,
      startmonat:    state.startMonth,
      startjahr:     state.startYear,
      umsatz:        state.revenue,
      transaktionen: state.transactions,
      startup:       state.isStartup ? 'ja' : 'nein',
      raten:         state.ratenCount,
      monatlich:     calc ? Math.ceil(calc.totalMonthly) : HOLDINGS_PRICE,
    });
    const sep = baseURL.includes('?') ? '&' : '?';
    return `${baseURL}${sep}${params.toString()}`;
  };

  /* ─── CTA TYPEFORM-PREFILL (ersetzt den früheren Confetti-Handler) ─── */
  /* Wendet die mit allen Kalkulator-Daten gebaute Typeform-URL auf den CTA an.
     Vorher hängte das im Confetti-Click-Handler — jetzt eigenständig in der IIFE. */
  function bindCtaPrefill() {
    const selectors = ['.summary-cta a', '.summary-cta', '.floating-bar-cta a', '.floating-bar-cta'];
    selectors.forEach(sel => {
      document.querySelectorAll(sel).forEach(cta => {
        if (cta.dataset.prefillBound) return;
        cta.dataset.prefillBound = 'true';
        cta.addEventListener('click', e => {
          const href = cta.getAttribute('href') || cta.closest('a')?.getAttribute('href') || '#';
          if (!href || href === '#') return;
          e.preventDefault();
          const url = window.bsteuernTypeformURL ? window.bsteuernTypeformURL(href) : href;
  ...
  function init() {
    ...
    renderPackageCards();
    updateSummary();
    startActionCountdown(); /* v37: Tarifaktion-Banner + Countdown */
