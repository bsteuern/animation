/* ════════════════════════════════════════════════════════════
   ...
   - aktionEnde()/aktionAktiv(): Aktion läuft bis 15. Juli 2026 23:59:59
...
(function () {
  ...
  /* Aktionsende: 15. Juli 2026, 23:59:59 (lokale Zeit) — Aktion läuft bis inkl. 15.07. */
  function aktionEnde() {
    return new Date(2026, 6, 15, 23, 59, 59, 999).getTime();  // Monat 0-indexiert: 6 = Juli
  }
  ...
  /* Baut die Typeform-URL mit allen Kalkulator-Daten als Hidden-Field-Parameter.
     Wird als window.bsteuernTypeformURL exposed, damit bindConfetti() (außerhalb
     der IIFE) darauf zugreifen kann, ohne den Closure zu brechen. */
  ...
  function fireConfetti(originX, originY) {
    ...
    (function draw() {
      ...
      pieces.forEach(p => {
        ...
        if (p.shape === 'streamer') ctx.fillRect(-p.size/2, -p.size/6, p.size, p.size/3);
  ...
  function bindConfetti() {
    ...
    selectors.forEach(sel => {
      document.querySelectorAll(sel).forEach(cta => {
        /* Nur einmal binden */
        if (cta.dataset.confettiBound) return;
        cta.dataset.confettiBound = 'true';
