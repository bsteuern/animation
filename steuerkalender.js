...
(function () {
  ...
  function collect() {
    ...
    dom.upcoming  = $('[data-termin-liste]', dom.root);
    dom.ics       = $('[data-kal-ics]', dom.root);
    dom.month     = $('[data-monat-titel]', dom.root);
    dom.range     = $('[data-monat-spanne]', dom.root);
    dom.grid      = $('[data-kal-grid]', dom.root);
    dom.agenda    = $('[data-agenda-liste]', dom.root);
    dom.agendaTtl = $('[data-agenda-liste-title]', dom.root);
  ...
  function setToggle(name, on) {
    var n = dom.root.querySelectorAll('[data-schalter-btn="' + name + '"]');
  ...
  function bind() {
    dom.root.addEventListener('click', function (ev) {
      var t = ev.target.closest('[data-schalter-btn], [data-kal-ust], [data-kal-nav], [data-kal-open], [data-kal-close], [data-kal-ics], .gitter-zelle.has-events');
      if (!t || !dom.root.contains(t)) return;

      /* Profil-Toggles */
      var tg = t.getAttribute('data-schalter-btn');
