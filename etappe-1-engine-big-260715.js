/* ════════════════════════════════════════════════════════════
   ...
   ÄNDERUNGEN v38:
   - Countdown/Tarifaktion (v37) vollständig entfernt: Banner, Floating-Bar-
     Streifen, alle aktion*()-Funktionen und der NEUE_PREISE-Override.
   - pkgPrice() nutzt jetzt direkt den CMS-Grundpreis (= aktueller Preis).
   - Onboarding 399 €; Transaktions-Staffel 50er (+25 €/50 ab 50); Umsatz-Staffel 10–70 %.
...
(function () {
  ...
  const TX_TIERS = [
    { upTo: 50,  cost: 0   }, { upTo: 100, cost: 25  }, { upTo: 150, cost: 50  },
    { upTo: 200, cost: 75  }, { upTo: 250, cost: 100 }, { upTo: 300, cost: 125 },
    { upTo: 350, cost: 150 }, { upTo: 400, cost: 175 }, { upTo: 450, cost: 200 },
    { upTo: 500, cost: 225 }, { upTo: 550, cost: 250 }, { upTo: 600, cost: 275 },
    { upTo: Infinity, cost: 300 }
  ];

  /* Rundumservice (vollservice): 449 € Grundpreis enthält KEINE Transaktionen.
     Jede Stufe kostet 75 € statt der 25 € bei Selbststarter/Vorsorge — kein
     Freikontingent. Bricht bewusst dieselben 50er-Grenzen wie TX_TIERS. */
  const TX_TIERS_VOLLSERVICE = [
    { upTo: 50,  cost: 75  }, { upTo: 100, cost: 150 }, { upTo: 150, cost: 225 },
    { upTo: 200, cost: 300 }, { upTo: 250, cost: 375 }, { upTo: 300, cost: 450 },
    { upTo: 350, cost: 525 }, { upTo: 400, cost: 600 }, { upTo: 450, cost: 675 },
    { upTo: 500, cost: 750 }, { upTo: 550, cost: 825 }, { upTo: 600, cost: 900 },
    { upTo: Infinity, cost: 975 }
  ];

  const REVENUE_TIERS = [
    { value: 0,       label: 'bis 250k (0%)',   percent: 0  },
    { value: 250000,  label: '250–500k (10%)',  percent: 10 },
    { value: 500000,  label: '500k–1Mio (20%)', percent: 20 },
    { value: 1000000, label: '1–2Mio (30%)',    percent: 30 },
    { value: 2000000, label: '2–3Mio (40%)',    percent: 40 },
    { value: 3000000, label: '3–4Mio (50%)',    percent: 50 },
    { value: 4000000, label: '4–5Mio (60%)',    percent: 60 },
    { value: 5000000, label: '>5Mio (70%)',     percent: 70 }
  ];

  /* STARTUP_TARIFS entfernt: Der "Gründer oder Startup"-Toggle ist reine
     UI (feinere Transaktions-Schrittweite unter 50, siehe getTxStep) —
     er hat keine Preiswirkung mehr. Diese Tabelle wurde nirgends gelesen
     (kein STARTUP_TARIFS[...]-Zugriff im Code) und war bereits totes
     Gewicht, bevor sie entfernt wurde. */
  ...
  const COMPARE_FEATURES = [
    ...
    { key: 'uva_prep',       label: 'Alles bereit für die USt-VA – Du übermittelst',         selbststarter: false,     sorglos: true,      vollservice: false     },
  ...
  /* ─── Aktueller Grundpreis eines Pakets (kommt aus dem Webflow-CMS) ─── */
  ...
  /* Zentraler Paket-Setter: Rundumservice startet ohne Transaktionen (0),
     da im 449-€-Grundpreis keine enthalten sind — jede zählt sofort mit
     75 €/50. Andere Pakete bleiben beim Standard-Default (100), inklusive
     der ersten 50 kostenlos. Reset nur bei echtem Paketwechsel, damit ein
     erneuter Klick auf die bereits gewählte Karte manuell eingegebene
     Transaktionen nicht überschreibt. */
  function setPackage(id) {
    const changed = state.package !== id;
    state.package = id;
    if (changed) {
      state.transactions = id === 'vollservice' ? 0 : 100;
    }
  }
  ...
  function txSurcharge(tx, pkgId) {
    if (tx <= 0) return 0; /* 0 Transaktionen = 0 Aufschlag, auch ohne Freikontingent */
    const tiers = pkgId === 'vollservice' ? TX_TIERS_VOLLSERVICE : TX_TIERS;
    return (tiers.find(t => tx <= t.upTo) || tiers.at(-1)).cost;
  }
  ...
  function calculate() {
    ...
    const txCost        = txSurcharge(state.transactions, pkg.id);
    const revTier       = getRevenueTier(state.revenue);
    const surcharge     = basePrice * (revTier.percent / 100); /* NICHT auf txCost — siehe Preisliste & Rechenlogik #04 */
  ...
  function renderPackageCards() {
    ...
    wrap.querySelectorAll('.package-card').forEach(card => {
      card.addEventListener('click', () => {
        ...
        setPackage(card.dataset.package);
        renderPackageCards();
        updateSummary();
  ...
  function updateSummary() {
    ...
    toggleEl($('sumLineStartupDiscount'), false); /* keine Preiswirkung mehr — Zeile bleibt immer versteckt */
  ...
  function bindForm() {
    ...
    tiles.forEach(tile => {
      tile.addEventListener('click', () => {
        ...
        if (gr) { const f = gr.packages.find(p => !p.comingSoon); if (f) setPackage(f.id); }
    ...
    $('step1PillClose')?.addEventListener('click', () => {
      state.form = 'einzelunternehmer'; setPackage('selbststarter');
