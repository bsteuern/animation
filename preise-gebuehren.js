...
(function () {
  ...
  /* ─── Fester, kurzer Smooth-Scroll statt nativem CSS scroll-behavior ───
     Safari/WebKit skaliert die native smooth-scroll-Dauer mit der
     Scroll-Distanz — auf einer langen Seite wie /preise fühlte sich ein
     Sprung von "Betrieblich" zu "Zusätzlich" dadurch wie ein Hänger an
     (mehrere Sekunden). Diese Funktion nutzt eine feste, kurze Dauer
     (ease-out), unabhängig von der Distanz. */
  function smoothScrollToY(targetY, duration) {
    ...
    function step(ts) {
      ...
      var y = startY + diff * easeOutCubic(t);
  ...
  function scrollToAnchor(hash) {
    ...
    var targetY = target.getBoundingClientRect().top + window.pageYOffset - scrollMarginTop;
    smoothScrollToY(targetY, 500);
  }
  window.bsScrollToAnchor = scrollToAnchor; /* global — wird von der dritten IIFE (Mobile-Sheet) genutzt */

  function initAnchorClicks() {
    /* Fängt jeden In-Page-Anker-Klick ab (Tabbar oben + Mini-Tabbar beim
       Hochscrollen) — deckt beide .anchor-link-Instanzen über die Klasse ab. */
...
(function(){
  ...
  document.querySelectorAll('.mobile-sheet-opt').forEach(function(opt) {
    opt.addEventListener('click', function() {
      ...
      closeSheet();
      // Scroll to section — feste, kurze Dauer statt nativem scrollIntoView
      // (dessen Safari/WebKit-Timing mit der Distanz skaliert)
      if (window.bsScrollToAnchor) window.bsScrollToAnchor(href);
    });
  });
