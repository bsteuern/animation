...
/* ════════════════════════════════════════════════════════════
 * b'steuern · Preise und Gebühren — Webflow-Integrationsschicht
 * Läuft VOR den beiden Render-/Interaktionsskripten der Seite.
 *
 *  1) --wf-nav-h live aus der Geometrie der globalen Navbar setzen
 *     (getBoundingClientRect().bottom, clamp ≥ 0). Funktioniert unter
 *     jedem Navbar-Verhalten — sticky, smart-nav/hide-on-scroll, fixed
 *     oder keins — ohne Framework-State zu lesen. Schließt die Lücke,
 *     wenn die Navbar beim Runterscrollen wegfährt, und folgt der
 *     variablen Navbar-Höhe (80 ↔ 64).
 *  2) Mobile-Selector (≤720px) als hide-on-scroll: blendet beim
 *     Hochscrollen ein, beim Runterscrollen aus — spiegelt die
 *     Desktop-Mini-Bar. Breakpoint-gegated, damit nie beide Bars
 *     gleichzeitig --nav-h beanspruchen.
 *  3) Toter #beratung-Link → kostenlose Beratung.
 * ════════════════════════════════════════════════════════════ */
(function () {
  var MOBILE_MAX = 720;       // muss zum CSS-Breakpoint passen
  var NAV_FALLBACK = 80;      // px, falls keine Navbar gefunden wird
  var DELTA = 6;              // Scroll-Hysterese gegen Flackern
  var root = document.documentElement;

  // ── Navbar finden (native Webflow-Navbar = .w-nav) ──
  function findNav() {
  ...
  // ── #beratung-Toter-Link reparieren ──
  ...
  // ── --wf-nav-h live setzen ──
  var lastNavH = -1;
  function updateNavH() {
    var h;
    if (navEl) {
      // sichtbarer unterer Rand der Navbar relativ zum Viewport
  ...
  // ── Mobile-Selector hide-on-scroll ──
  var sel = document.querySelector('.mobile-selector');
  var lastY = window.scrollY;
  function updateSelector() {
    if (!sel) return;
    if (window.innerWidth > MOBILE_MAX) {       // nur ≤720px aktiv
      sel.classList.remove('is-visible');
      return;
    }
    var y = window.scrollY;
    // erst zeigen, wenn man ein Stück gescrollt ist (nicht direkt am Seitenanfang)
    var pastTop = y > (lastNavH + 40);
    if (!pastTop) {
      sel.classList.remove('is-visible');
    } else if (y < lastY - DELTA) {             // hochscrollen → zeigen
      sel.classList.add('is-visible');
    } else if (y > lastY + DELTA) {             // runterscrollen → verstecken
      sel.classList.remove('is-visible');
    }
  }
  ...
  function init() {
    ...
    // Navbar-Höhe kann sich nach Font-/Layout-Load noch ändern
...
(function () {
  ...
  const dataBetrieblich = [
    ...
    { sec: 'Zusätzliche Leistungen', meta: 'optional', rows: [
      { title: 'Jährliche Services für Vor- und Altjahre',
        ...
        v: ['ab 1.788 €*', 'ab 2.388 €*', 'Bald verfügbar'] },
    ]},
  ...
  function renderTable(targetId, groups, idealRow) {
    ...
    groups.forEach(function (g, idx) {
      ...
      rowIdx = 0; // alternation resets per section
  ...
  var dataLohn = [
    ...
    { sec: 'Erstanlage von Mitarbeitern', meta: 'pro Anlage', rows: [
      { title: 'Erstanlage Mitarbeiter',
        desc: 'Wir übernehmen die Erstanlage – der entsprechende Fragebogen muss vollständig ausgefüllt werden.',
        v: '' },
      { title: 'bis 10 Mitarbeiter', v: '99 €*' },
      { title: 'bis 20 Mitarbeiter', v: '199 €*' },
      { title: 'ab 20 Mitarbeiter', v: 'auf Anfrage' },
    ]},
  ...
  var dataExtra = [
    ...
    { sec: 'Gründung', meta: 'einmalig', rows: [
      { title: 'Beantragung USt-ID', v: '49 €*' },
      { title: 'Steuerlicher Erfassungsbogen', v: '199 €*' },
      { title: 'EB-Bilanz bei Neugründung', v: '149 €*' },
    ]},
    { sec: 'Bescheinigungen', meta: 'pro Bescheinigung', rows: [
      { title: 'Einkommensteuernachweis / EÜR', v: '49 €*' },
    ...
    { sec: 'Zusammenarbeit', meta: 'Pauschalen', rows: [
      ...
      { title: 'Expresszuschlag', desc: 'unter 6 Wochen Bearbeitungszeit', v: '299 €*' },
    ...
    { sec: 'Andere Dienstleistungen', meta: 'einzeln', rows: [
      ...
      { title: 'Ersteintrag & Änderungen im Transparenzregister', v: '99 €*' },
...
(function(){
  ...
  // Sync mobile state with scroll position (IntersectionObserver on sections)
