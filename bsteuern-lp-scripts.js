...
(function(){
  ...
  function open(i){
    ...
    var grid=list.closest('.why-grid')||list; grid.style.setProperty('--line','color-mix(in srgb,'+colorOf(i)+' 55%,#fff)');
    active=i;
  ...
  items.forEach((it,i)=>{
    const q=it.querySelector('.why-q');
    q.addEventListener('click',()=>open(i));
    q.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open(i);}});
  });
