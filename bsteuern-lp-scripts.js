...
(function(){
  ...
  tabs.forEach(t => t.addEventListener('click', () => {
    ...
    document.getElementById('prFeat').innerHTML = d.feat.map(g =>
      '<div class="pr-feat-grp"><div class="pr-feat-head">' + g[0] + '</div><ul class="pr-feat-list">' + g[1].map(i => '<li>' + i + '</li>').join('') + '</ul></div>'
    ).join('');
  }));
})();
...
(function(){
  ...
  if(!list)return;
  const items=[...list.querySelectorAll('.why-item')];
  const imgs=[...media.querySelectorAll('.why-img')];
  ...
  function setBar(item,run){
    const bar=item.querySelector('.why-bar');
  ...
  function open(i){
    items.forEach((it,idx)=>{
      ...
      if(idx!==i){const b=it.querySelector('.why-bar');b.style.transition='none';b.style.width='0%';}
    });
  ...
  items.forEach((it,i)=>it.querySelector('.why-q').addEventListener('click',()=>open(i)));
  list.addEventListener('mouseenter',()=>{paused=true;clearTimeout(timer);
    const bar=items[active].querySelector('.why-bar');
...
(function(){
  const steps=[...document.querySelectorAll('#svcSteps .svc-step')];
  const stage=document.getElementById('svcStage');
  if(!steps.length||!stage)return;
  steps.forEach((step,i)=>{
    const mock=document.createElement('div');
    mock.className='svc-mock'+(i===0?' show':'');
    mock.setAttribute('style',step.getAttribute('style')||'');
    mock.innerHTML=step.querySelector('.svc-vis').innerHTML;
    stage.appendChild(mock);
  });
  const mocks=[...stage.querySelectorAll('.svc-mock')];
  let current=-1;
  function update(){
...
(function(){
  ...
  document.querySelectorAll('.shape-trans').forEach(el => {
    const shape = el.dataset.shape || 'square';
    const a = el.dataset.colorA || '#F9F4EC';
    const b = el.dataset.colorB || '#FFFFFF';
    const density = parseFloat(el.dataset.density) || 0.4;
    const direction = el.dataset.direction || 'uniform';
    el.style.setProperty('--ta', a);
    el.style.setProperty('--tb', b);
    const rowD = direction === 'top-to-bottom'
      ? [Math.min(1, density * 1.35), Math.max(0.05, density * 0.65)]
      : [density, density];
    let html = '';
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        const filled = Math.random() < rowD[r];
        if (filled) {
          let color;
          color = r === 0 ? 'b' : 'a';
          html += '<span class="cell filled" data-color="' + color + '"></span>';
        } else {
