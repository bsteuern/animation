...
(function () {
  ...
  var current     = null;
  var selectedPkg = 'sorglos';
  var detailsOpen = false;

  /* ── Initialzustand erzwingen ──────────────────────────────
     Webflow ignoriert das hidden-Attribut als Custom Attribute
     wenn es selbst display:block als Inline-Style setzt.
     Wir überschreiben das direkt beim Scriptstart, bevor der
     Browser das erste Frame rendert. ── */
  if (card)    { card.style.setProperty('display', 'none', 'important'); }
  if (discSec) { discSec.style.setProperty('display', 'none', 'important'); }
  if (hint)    { hint.style.removeProperty('display'); } /* hint ist sichtbar per default */
  ...
  function render() {
    ...
    if (rcAdvisor) {
      ...
      rcAdvisor.style.setProperty('display', 'flex', 'important');
