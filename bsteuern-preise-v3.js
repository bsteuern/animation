...
(function () {
  ...
  /* rcAdvisor initial verstecken — JS setzt display:flex beim ersten render() */
  if (rcAdvisor) rcAdvisor.style.setProperty('display', 'none', 'important');
  ...
  function render() {
    ...
    if (rcAdvisor) { rcAdvisor.style.setProperty('display', 'flex', 'important'); rcAdvisor.innerHTML = advisorHtml(current); }
  ...
  /* Click-Handler auf document delegiert — fängt Klicks auf .founder
     und alle Kind-Elemente zuverlässig ab */
  document.addEventListener('click', function (e) {
    var foundEl = e.target.closest('.founder');
    if (foundEl && founderCb) {
      /* Input selbst wurde geklickt — Browser hat checked schon getoggelt */
      /* Anderes Kind wurde geklickt — wir müssen checked selbst togglen */
      if (e.target !== founderCb) {
